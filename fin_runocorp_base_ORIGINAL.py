#!/usr/bin/env python3
"""
Omorfi HFST Lemmatizer with Voikko Normalization - V17 PHASE 9 REFACTORED (Morphological Features in Tier 1)

NEW IN V17 Phase 9 REFACTORED:
...
- V17 Phase 9: Expected 60.5-61.5% accuracy (+2-3pp) ← CURRENT

Usage:
    python3 omorfi_hfst_with_voikko_v17_phase9.py < input.txt
"""
...
print(f"[omorfi_v17_phase9] Loaded {VERSION}", file=sys.stderr)
...
class OmorfiHfstWithVoikkoV16Hybrid:
    ...
    def __init__(self, model_dir: str = None, voikko_path: str = None, lang: str = 'fi'):
        """Load Omorfi HFST models, Voikko, Stanza, and V16 hybrid lexicon"""
        if model_dir is None:
            model_dir = os.path.expanduser('~/.omorfi')

        self.model_dir = model_dir
        self.lang = lang
        self.describe_path = f'{model_dir}/omorfi.describe.hfst'
        ...
        if LIBVOIKKO_AVAILABLE:
            ...
            try:
                sukija_path = os.path.expanduser("~/.voikko/sukija/5/mor-sukija")
            ...
            try:
                old_finnish_path = os.path.expanduser("~/.voikko/5/5")
        ...
        if voikko_path is None and not self.voikko:
            ...
            self.voikko_path = voikko_path if not self.voikko else None
    ...
    def parse_omorfi_tags(self, analysis: str) -> Dict:
    ...
    def rank_voikko_suggestions(self, word: str, suggestions: List[str], stanza_upos: str = None) -> List[Tuple[str, float]]:
        """
        ...
        for suggestion in suggestions:
            ...
            if self.dialectal_normalizer:
                ...
                score += edit_dist * 2.0  # Weight edit distance heavily
            else:
                # Fallback to simple character difference
                score += abs(len(word) - len(suggestion)) * 0.5
                ...
                if analyses:
                    for analysis, _ in analyses:
                        ...
                        if tags['pos'] and stanza_upos in OMORFI_POS_UD.get(tags['pos'], set()):
                            score -= 1.5  # Strong bonus for POS match
                            break
            ...
            if self.pos_aware_lexicon and suggestion.lower() in self.pos_aware_lexicon:
                score -= 0.8  # Bonus for being in training data

            # 4. Proper noun penalty (unless expected)
            if suggestion and suggestion[0].isupper():
                if stanza_upos == "PROPN":
                    score -= 0.5  # Bonus if proper noun expected
                else:
                    score += 1.0  # Penalty if proper noun not expected

            # 5. Length similarity bonus
            len_diff = abs(len(word) - len(suggestion))
            if len_diff <= 2:
                score -= 0.3  # Small bonus for similar length
    ...
    def lemmatize_sentence(self, tokens: List[str]) -> List[Dict]:
        """
    ...
    def select_best_alternative(self, word: str, analyses: List[Tuple[str, float]]) -> Tuple[str, float, str, List[Dict]]:
        ...
        for analysis, weight in analyses[:5]:  # Consider top 5
            ...
            if lemma:
                ...
                adjusted_score = base_score - (feature_bonus * 3.0)  # Scale feature bonus
    ...
    def get_ufeats_from_analysis(self, analysis_str: str, weight: float = 0.0) -> Optional[Dict]:
        """
        V17 Phase 9: Extract Universal Features from Omorfi HFST analysis string

        Parses HFST format directly: [KEY=VALUE][KEY=VALUE]...

        HFST tags mapped to Universal Features:
        - NUM=SG → Number=Sing
        - NUM=PL → Number=Plur
        - CASE=NOM/GEN/PAR/INE/... → Case=Nom/Gen/Par/Ine/...
        - TENSE=PAST/PRES → Tense=Past/Pres
        - MOOD=INDV/COND/IMPV → Mood=Ind/Cond/Imp
        - VOICE=ACT/PASS → Voice=Act/Pass
        - PERS=SG1/SG2/SG3/PL1/PL2/PL3 → Person=1/2/3, Number=Sing/Plur
        - INF=A/E/MA/MINEN → VerbForm=Inf, InfForm=1/2/3/4

        Args:
            analysis_str: Raw HFST analysis (e.g., '[WORD_ID=kissa][UPOS=NOUN][NUM=SG][CASE=NOM]')
            weight: Analysis weight (unused, kept for compatibility)

        Returns:
            Dict of Universal Features or None if parsing fails
        """
        if not analysis_str:
            return None
    ...
    def compute_feature_similarity(self, feats1: Optional[Dict], feats2: Optional[Dict]) -> float:
        """
        V17 Phase 9: Compute morphological feature similarity score

        Compares two Universal Feature dicts and returns a similarity bonus.
        Higher bonus = more similar morphological features.

        Key features compared:
        - Case agreement: +0.3 bonus
        - Number agreement: +0.2 bonus
        - Tense agreement (for verbs): +0.2 bonus
        - VerbForm agreement: +0.2 bonus
        - Degree agreement (for adjectives): +0.1 bonus

        Args:
            feats1: Universal Features dict from word form
            feats2: Universal Features dict from candidate lemma

        Returns:
            Similarity bonus (0.0-1.0), higher = more similar
        """
        ...
        # Case agreement (very important for Finnish)
        if feats1.get('Case') and feats2.get('Case'):
            if feats1['Case'] == feats2['Case']:
                similarity += 0.3

        # Number agreement (singular vs plural)
        if feats1.get('Number') and feats2.get('Number'):
            if feats1['Number'] == feats2['Number']:
                similarity += 0.2

        # Tense agreement (for verbs)
        if feats1.get('Tense') and feats2.get('Tense'):
            if feats1['Tense'] == feats2['Tense']:
                similarity += 0.2

        # VerbForm agreement (Inf, Fin, Part, etc.)
        if feats1.get('VerbForm') and feats2.get('VerbForm'):
            if feats1['VerbForm'] == feats2['VerbForm']:
                similarity += 0.2

        # Degree agreement (for adjectives: Pos, Cmp, Sup)
        if feats1.get('Degree') and feats2.get('Degree'):
            if feats1['Degree'] == feats2['Degree']:
                similarity += 0.1
    ...
    def lemmatize_with_fuzzy_lexicon_morphological(self, word: str, stanza_upos: str = None) -> Optional[Dict]:
        ...
        for lexicon_word, pos_dict in self.pos_aware_lexicon.items():
            ...
            if distance <= 2.0:  # Standard fuzzy threshold
    ...
    def lemmatize_with_fuzzy_lexicon_aggressive(self, word: str, stanza_upos: str = None) -> Optional[Dict]:
        ...
        for test_variant in all_variants:
            for lexicon_word, pos_dict in self.pos_aware_lexicon.items():
                ...
                if distance <= 3.5:  # Relaxed threshold
                    for pos, entry in pos_dict.items():
                        # Filter by POS if available
                        if stanza_upos and pos != stanza_upos:
                            continue
    ...
    def lemmatize(self, word: str) -> Dict:
        ...
        # Tier 6: Morphology-Aware Fuzzy Lexicon (V17 Phase 9) - NEW
        ...
        # Tier 7: Aggressive Fuzzy Lexicon (V17 Phase 8)
        ...
        # Tier 8: Identity fallback (V17 Phase 1.5)
    ...
    def lemmatize_batch(self, words: List[str]) -> Dict:
        """
