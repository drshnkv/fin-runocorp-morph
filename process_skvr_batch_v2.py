...
"""
========================================================
BATCH CSV PROCESSOR V2 - Lemmatize with Finnish Dialectal Dictionary
========================================================

This script processes large CSV files containing full poems and lemmatizes
them using the V17 Phase 10 lemmatization system with dialectal dictionary integration.

NEW IN V2:
- Integrates 19,385 validated dialectal variants from Suomen Murteiden Sanakirja (SMS)
- Confidence-based dictionary override for spelling correction guesses
- Expected accuracy improvement: 58.8% → 59.0%+ with dialectal coverage
- Supports both train-only and combined lexicons via --lexicon-path
...
USAGE:
# Using train-only lexicon + dialectal dictionary (default)
...
from fin_runocorp_base_v2_dialectal_dict_integrated import OmorfiHfstWithVoikkoV16Hybrid, LemmatizerConfig
...
def process_batch(args):
    ...
    # STEP 1: Setup
    print("=" * 80)
    print("V17 PHASE 10 BATCH CSV PROCESSOR - Finnish Runosong Lemmatization (with Dialectal Dictionary)")
    print("=" * 80)
    print()
    ...
    # STEP 3: Load lemmatizer with V2 configuration
    print("Loading V17 Phase 10 lemmatizer (with dialectal dictionary)...", file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Batch process large CSV files with V17 Phase 10 Finnish lemmatization (with dialectal dictionary)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process entire file with progress tracking (train-only lexicon + dialectal dictionary)
  python3 process_skvr_batch_v2.py --input skvr_runosongs_okt_2025.csv --output results_v2.csv

  # Use combined train+test lexicon for better coverage (recommended)
  python3 process_skvr_batch_v2.py --input skvr_runosongs_okt_2025.csv --output results_v2.csv \\
    --lexicon-path selftraining_lexicon_v16_min1_combined.json

  # Resume interrupted processing
  python3 process_skvr_batch_v2.py --input skvr_runosongs_okt_2025.csv --output results_v2.csv --resume

  # Smaller chunks for faster saves
  python3 process_skvr_batch_v2.py --input skvr_runosongs_okt_2025.csv --output results_v2.csv \\
    --chunk-size 50 --save-interval 60

  # Use custom dialectal dictionary
  python3 process_skvr_batch_v2.py --input skvr_runosongs_okt_2025.csv --output results_v2.csv \\
    --dialectal-dict-path custom_dialectal_index.json
        """
    )
    ...
    parser.add_argument('--dialectal-dict-path', type=str, default=None,
                       help='Path to dialectal dictionary JSON file (default: sms_dialectal_index_v4_final.json)')
